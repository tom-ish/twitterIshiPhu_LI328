			...
			success: function(rep){
						if(rep.postTweet == "success!"){
							...
							$("#tweetZone").prepend("<br/>" +
									...
									"<img class = \"closeButton\" src=\"../img/button_close.png\" alt=\"" + c.id + "\">" +
...
function showTimeline(tps){
	...
	jQuery('#header').prepend("<button id = \"favori\" class = \"needed\">Favori</button>");
	jQuery('#favori').append("<span id = \"#favoriList\">Favori</span>");
	
	$("#favoriList").css("top","40px");
	$("#favoriList").css("left","0px");
	$("#favoriList").css("height","50%");
